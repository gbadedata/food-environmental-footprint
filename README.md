...
**Data licence & attribution.** The **code** here is MIT-licensed (see `LICENSE`). The **data is not mine**: it originates with **Poore & Nemecek (2018), *Science*** (doi:10.1126/science.aaq0216) - please cite the authors if you reuse it. A reusable version is republished by Our World in Data under **CC BY 4.0**; the original article is © AAAS. The CSV here came via a community mirror (see the source note above), so treat it as illustrative rather than the canonical supplementary data.
